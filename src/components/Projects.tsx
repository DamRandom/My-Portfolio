        ...
        {projects.map(({ title, image, description, github, vercel, tags }) => (
          ...
          >
            ...
            <div className="relative shadow-lg border border-white/20 overflow-hidden w-full max-w-[300px] flex-shrink-0">
              ...
              <div
                className={`
                  absolute inset-0 backdrop-blur-sm transition-opacity duration-300 hover:opacity-0 flex justify-center items-center
                  ${title === "Rose-Reverie"
                    ? "bg-[#501823]/40"
                    : title === "PowerHaus"
                    ? "bg-[#D36112]/40"
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {tags.map((tag) => (
                  <motion.button
                    ...
                    className="px-3 py-1 text-xs sm:text-sm font-medium backdrop-blur-sm focus:outline-none focus:ring-2 focus:ring-[#06061B] focus:ring-offset-1 rounded-none border border-transparent cursor-default"
                  >
                    {tag}
                  </motion.button>
                ))}
